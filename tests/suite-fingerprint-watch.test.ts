...
test('suite fingerprint includes optional sticker runtime files without changing older suites', async t => {
  const root = await createFixture()
  t.after(async () => removeFixture(root))
  const options = { cache: new FingerprintCache(), gitBin: 'fixture-git-does-not-exist' }
  const before = await computeSuiteFingerprint(root, options)
  const directory = join(root, 'amphoreus/assets/stickers')
  await mkdir(join(directory, 'nested'), { recursive: true })
  await writeFile(join(root, 'amphoreus/scripts/unrelated.py'), 'excluded')
  await writeFile(join(directory, 'fixture.png'), 'excluded')
  await writeFile(join(directory, 'nested/fixture.webp'), 'excluded')
  assert.equal((await computeSuiteFingerprint(root, options)).manifestSha256, before.manifestSha256)
  await writeFile(join(root, 'amphoreus/scripts/stickers.py'), 'print("stickers")')
  await writeFile(join(directory, 'manifest.json'), '{"version":1}')
  await writeFile(join(directory, 'fixture.webp'), 'RIFF0000WEBPfixture')
  const added = await computeSuiteFingerprint(root, options)
  assert.equal(added.fileCount, before.fileCount + 3)
  assert.notEqual(added.manifestSha256, before.manifestSha256)
  const paths = added.manifest!.map(entry => entry.rel)
  assert.ok(paths.includes('amphoreus/scripts/stickers.py'))
  assert.ok(paths.includes('amphoreus/assets/stickers/manifest.json'))
  assert.ok(paths.includes('amphoreus/assets/stickers/fixture.webp'))
  assert.equal(paths.some(path => path.includes('nested') || path.endsWith('.png')), false)
  await writeFile(join(directory, 'fixture.webp'), 'RIFF0000WEBPchanged image')
  const changed = await computeSuiteFingerprint(root, options)
  assert.notEqual(changed.manifestSha256, added.manifestSha256)
  await unlink(join(root, 'amphoreus/scripts/stickers.py'))
  await unlink(join(directory, 'manifest.json'))
  await unlink(join(directory, 'fixture.webp'))
  assert.equal((await computeSuiteFingerprint(root, options)).manifestSha256, before.manifestSha256)
})
