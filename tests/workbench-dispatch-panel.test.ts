...
test('state and enter-seat messages recompute suggestions, defer full replacement, and settle dispatch RPC', () => {
  ...
  assert.match(source, /data\.type === 'amphoreus:dispatched'/u)
  assert.match(source, /data\.type === 'amphoreus:conference-started'/u)
  assert.match(source, /data\.type === 'amphoreus:handoff-accepted'/u)
})
