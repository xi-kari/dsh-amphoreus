...
| 路由 | `PUT|DELETE /amphoreus/api/seat-sound/<heroId>/<slot>` | PUT 二进制正文，`content-type` 为音频 MIME；或 `application/octet-stream` + `x-amphoreus-ext: mp3…`（Windows 上 .ogg/.flac 的 `File.type` 常为空）。需 `x-amphoreus-nonce`。415 未知类型 / 413 超限 / 400 非法 hero 或槽位 / 503 无 dataDir。 |
...
`audio/mpeg` mp3 · `audio/ogg` ogg · `audio/wav`/`audio/x-wav` wav · `audio/webm` webm · `audio/mp4` m4a · `audio/aac` aac · `audio/flac` flac。单文件上限 20 MiB，超过即 413，临时文件已删除、不发布任何记录。
