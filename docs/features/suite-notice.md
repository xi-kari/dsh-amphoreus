...
**唯一需要重启的情形：** 进程启动时技能根不存在（boot `level === 'L3'`）。此时 `resolver.start()` 没有创建 watcher，`scheduleReparse()` / `forceReparse()`（POST `/amphoreus/api/reparse`）都是空操作，只有重启 web 进程才会重新扫描。
...
| `missing` | 变为 L3 或 `suite` 为空 | 套件未识别 |
| `recovered` | 从 L1/L2/L3 回到 L0 | 套件已恢复，当前 {label} |

附加行：
- `suite.sessionsStale`（"已有 {n} 个会话在 /clear、恢复或新建后才会用到新卡"）：`staleSessions > 0` 且 kind ≠ missing 时显示。`staleSessions` = `injection.state==='done' && boundAt < suite.parsedAt` 的绑定数。
- `suite.restartHint`：仅当 `window.__AMPHOREUS_BOOT__.level === 'L3'`（启动时缺根）时显示；此时"重新解析"按钮也隐藏，因为宿主没有 watcher，点了也没用。

沉默规则：首个 ready 状态（或 boot）只作基线，不提示；强制重新解析但 sha 不变（仅 generation 递增）不提示；boot level 与首拉 level 不一致（例如启动 L3、首拉 L0）按转换分类提示一次。

## 动作

- **关闭**：`store.dismiss(id)`，id = `${level}:${sha}`，持久化到 `sessionStorage['dsh-amphoreus:suite-notice']`（数组，最多 32 条），同一页会话内不再弹同一条；storage 不可用或损坏时退化为内存去重。
- **重新解析**：仅 degraded / missing 且非启动缺根时出现，调用 `model.reparse()`（POST `/amphoreus/api/reparse`，nonce 头由 model 自带）。
...
- `src/client/index.ts`：`@anchor client-imports` 后导入；`@anchor client-services` 后 `createSuiteNoticeStore({ model, boot, storage: safeSessionStorage() })` + `ctx.effect` 释放；`@anchor shell-overlay-entries` 后在**同一个** `shell.overlay` inject 回调数组内 `ctx.slots.register({ id: 'amphoreus-suite-notice', order: 10, locale: NS, inject: () => ({ store, model, portalOpen, subscribePortal }) }, SuiteNoticeBanner)`。`ctx.slots.inject` 调用列表未变，assembly 测试原样通过。
...
`tests/client-suite-notice.test.ts`（13 项）：分类函数；首个 ready 不提示；sha 变化提示；同 sha 仅 generation 变化沉默；degraded → missing → recovered 序列；boot L3 基线与 `startedMissing`；关闭持久化 / 幂等 / 跨 store 生效；损坏或抛错的 storage 退化；`staleSessions` 计数；dispose 退订；横幅源码 regex（role=status、门户门控、无 ctx/fetch/appendChild、无防火墙词、键集对齐）；CSS 仅 dsw 令牌；index 接线。
