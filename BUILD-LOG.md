...
- 用户反馈四条（同日晚）并已处理：① 设置窗改为**全局半透明**（不随席位 frost，深色下 tint 混白提亮，模态遮罩变薄纱，左导航独立浅条，控制台内卡片变薄玻璃）；② 控制台去掉「打开画布工作台」外链（裸 synapse 页是异质 UI，工作台只在 Tab／总览里）；控制台重排：δ 水印题头、指标胶囊、13 席磁贴（席色顶条）；③ 每席独立 **正文墨色**（`ink.light/dark`）与**输入框纸色**（`composer.tintLight/Dark`），经 `--amph-ink`／`--amph-composer` 作用于对话流与文本框；④ 深色整体偏黑 → 深色玻璃配方把 tint 向白混 16–20%。另修：昔涟环境动效「涟漪」原用 inset box-shadow 画成了一个巨大实心圆环（用户截图指出），改为 1px radial-gradient 细线；星盘同法。
- 用户反馈：目录工作区（如“缇宝”）无法删除；全席征询后十三段席位会话同时挂在“我的目录”和“黄金裔席位”。修：目录行加“移除目录”（官方 `workspaces.delete`，仅删注册）；目录只列未绑定席位的会话，绑定会话计数一行小字提示。
- 终稿收尾三项：门户席卡下方材质样卡（该席圆角/rim 线型/tint 的小面板 + 风格名，`WorkspaceSeat.grammar`）；白厄侧栏玻璃片顶部半圆拱（仅展开态）；对话每轮右上 hover 显示 `NN` 页码（counter，`content / ""` 读屏静音）。
...
- 0.3.0 发布完成（2026-09-06）：commit/tag = `5f3c468e8813d8e5c279452487675d874329d976`。CI 首跑三次红（ubuntu/windows 同一处 TS7006）：本机 node_modules 为 junction 指向 dev 源码树，d.ts 隐式携带 Context 合并，npm 发布包需显式 `SettingsNamespaceView` 注解；以临时目录 `npm ci --legacy-peer-deps` 复现后修入 `a6f3ffe`，run `34000458048` 双绿。非 TTY `npm publish` 遇 EOTP，用户在 TTY 经 WebAuthn 完成，CLI 字面 `+ dsh-amphoreus@0.3.0`；registry gitHead 与 tag 相同，108 文件 / 352,433 B / unpacked 1,371,865 B，shasum `a58576714b12fbd836a8cedc461047081bfb4624`，本地 pack 与远端 tarball sha256 一致。tag 推送触发 release run `34002755334` success（重复发布被跳过）。GitHub Release `v0.3.0` 已建。套件 pin 升至 δ-me13 skill v1.7.0 `0594030`；表情按 manifest 扩展名服务 WebP/GIF/PNG（`5f3c468`）。事故：清理 worktree 时 PowerShell 递归删除穿过 junction，删掉 dev 树 `vendor/cordis` 与 `vendor/include` 的 lib/LICENSE/bin.js，已 `git checkout` + `tsc -b` + `tsdown --filter` 复原并冷启动验证。
