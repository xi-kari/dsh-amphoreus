...
export interface SeatPresetPanelProps {
  ...
  readonly onSave: (skillName: string, preset: SeatPreset | null) => void
}
...
export function SeatPresetPanel({ seats, seatNames, directory, busy, t, onSave }: SeatPresetPanelProps) {
  const [dir, setDir] = useState<DirectoryState>({ status: 'loading', presets: [], catalog: undefined, restorable: false })
  ...
  useEffect(() => {
    ...
    void Promise.all([directory.listAgentPresets(), directory.modelCatalog()]).then(
      ([presets, catalog]) => {
        if (cancelled) return
        setDir({ status: 'ready', presets, catalog, restorable: directory.canRestoreDefaultModel() })
      },
      () => {
        if (cancelled) return
        setDir(current => ({ ...current, status: 'error', restorable: directory.canRestoreDefaultModel() }))
      },
    )
  ...
  return (
    <section className={css.panel} aria-labelledby="amphoreus-seat-presets" data-amph-seat-presets="">
        ...
        : (
          <ul className={own.list}>
            {visible.map(seat => {
              ...
              const save = (tier: SeatPresetTier, value: string): void => { onSave(seat.skillName, withTier(preset, tier, value)) }
              return (
                <li
                  key={seat.skillName}
                  className={own.row}
                ...
                >
                  ...
                  <div className={own.fields}>
                    <label className={own.field}>
                      ...
                      <select className={own.select} disabled={disabled} value={preset?.agentPreset ?? ''} onChange={event => { save('agentPreset', event.currentTarget.value) }}>
                    ...
                    <label className={own.field}>
                      ...
                      <select className={own.select} disabled={disabled} value={modelValue} onChange={event => { save('model', event.currentTarget.value) }}>
                        <option value="">{t('settings.presetDefault')}</option>
                    ...
                    {efforts.length === 0 ? null : (
                      <label className={own.field}>
                        ...
                        <select className={own.select} disabled={disabled} value={preset?.model?.reasoningEffort ?? ''} onChange={event => { save('reasoningEffort', event.currentTarget.value) }}>
                          <option value="">{t('settings.presetDefault')}</option>
                    ...
                    <label className={own.field}>
                      ...
                      <select className={own.select} disabled={disabled} value={preset?.permission ?? ''} onChange={event => { save('permission', event.currentTarget.value) }}>
                        <option value="">{t('settings.presetDefault')}</option>
      ...
      {dir.status === 'error'
        ...
        : null}
      {usesModel
        ? <p className={`${own.note} ${dir.restorable ? '' : own.warn}`}>{t(dir.restorable ? 'settings.presetModelRestore' : 'settings.presetModelWarning')}</p>
        : null}
      <p className={own.note}>{busy ? t('settings.presetSaving') : t('settings.presetBlankOnly')}</p>
    </section>
  )
}
